...
# === Prompt Templates ===
DNA_OPTIMIZATION_PROMPT_TEMPLATE = """You are an expert in DNA enhancer and promoter design. You are given {num_seqs} DNA sequences, each has {seq_length} base pairs in length, along with their associated property values in five cell lines: K562, HepG2, GM12878, SKNSH, and A549. Your task is to mutate each DNA sequence, using only the nucleotides A, T, G, C, and inert {cell_line}-specific motifs as DNA sequences, to make them as {cell_line}-specific promoters with stability and diversity. {specific_task_instruction} Your response should include {num_seqs} DNA sequences, each exactly {seq_length} base pairs long (no need for optimization), and wrapped in <DNA> ... </DNA> tags.
...
DNA_OPTIMIZATION_PROMPT_NODIV_TEMPLATE = """You are an expert in DNA enhancer and promoter design. You are given {num_seqs} DNA sequences, each has {seq_length} base pairs in length, along with their associated property values in five cell lines: K562, HepG2, GM12878, SKNSH, and A549. Your task is to mutate each DNA sequence, using only the nucleotides A, T, G, C, and inert {cell_line}-specific motifs as DNA sequences, to make them as {cell_line}-specific promoters. {specific_task_instruction} Your response should include {num_seqs} DNA sequences, each exactly {seq_length} base pairs long (no need for optimization), and wrapped in <DNA> ... </DNA> tags.
...
@register_module("llm_dna_promoter_optimizer", "1.0.0")
class LlmDnaPromoterOptimizer(OptimizerModule):
    """
    Optimizer that uses an LLM to iteratively mutate and improve DNA promoter sequences.
    """
    # The objectives that this optimizer requires, and we know they are regular objectives
    REQUIRED_OBJECTIVE_NAMES = ["dna_diversity", "dna_stability", 'dna_motif_num', 'dna_motif_num_human', 'dna_hepg2_promoter_logfoldchange', 'dna_k562_promoter_logfoldchange', 'dna_sknsh_promoter_logfoldchange', 'dna_gm12878_promoter_logfoldchange', 'dna_a549_promoter_logfoldchange', 'dna_hepg2_promoter_tissue_cage_expression', 'dna_k562_promoter_tissue_cage_expression', 'dna_sknsh_promoter_tissue_cage_expression', 'dna_gm12878_promoter_tissue_cage_expression', 'dna_a549_promoter_tissue_cage_expression', ]
    ...
    def __init__(
        ...
        k562_scorer = get_scorer("dna_k562_promoter_logfoldchange")
        hepg2_scorer = get_scorer("dna_hepg2_promoter_logfoldchange")
        gm12878_scorer = get_scorer("dna_gm12878_promoter_logfoldchange")
        sknsh_scorer = get_scorer("dna_sknsh_promoter_logfoldchange")
        a549_scorer = get_scorer("dna_a549_promoter_logfoldchange")
        assert k562_scorer is not None, "K562 scorer is not found"
        assert hepg2_scorer is not None, "HepG2 scorer is not found"
        assert gm12878_scorer is not None, "GM12878 scorer is not found"
        assert sknsh_scorer is not None, "SKNSH scorer is not found"
        assert a549_scorer is not None, "A549 scorer is not found"
        
        self._filtering_objectives = {
             "K562": Objective(name="dna_k562_promoter_logfoldchange", description="", population_wise=False, scorer=k562_scorer),
            "HepG2": Objective(name="dna_hepg2_promoter_logfoldchange", description="", population_wise=False, scorer=hepg2_scorer),
            "GM12878": Objective(name="dna_gm12878_promoter_logfoldchange", description="", population_wise=False, scorer=gm12878_scorer),
            "SKNSH": Objective(name="dna_sknsh_promoter_logfoldchange", description="", population_wise=False, scorer=sknsh_scorer),
            "A549": Objective(name="dna_a549_promoter_logfoldchange", description="", population_wise=False, scorer=a549_scorer),
        }

    def _create_initial_population(self) -> Population:
        # Generate random sequences
        start_seq = grelu.sequence.utils.generate_random_sequences(
            n=self.num_initial_seqs,
            seq_len=self.seq_length,
            seed=0,
            output_format="strings"
        )
        return Population(candidates=[Candidate(representation=seq) for seq in start_seq])
    
    def _extract_cell_types(self, high_level_goal: str) -> List[str]:
        prompt = CELL_TYPE_PROMPT_TEMPLATE.format(high_level_goal=high_level_goal)
        count = 0
        try:
            while True:
                response = self.call_llm_with_prompt(prompt)['content']
    ...
    def optimize(
        ...
        **additional_info: Dict[str, Any]
        ...
        if high_level_goal in self._goals_to_cell_lines:
            ...
            cell_lines_to_optimize = self._extract_cell_types(high_level_goal)
        ...
        while True:
            # Filter the initial population based on the required objectives
            current_population = self._filter_candidates(current_population, cell_lines_to_optimize)
        ...
        # population = self._filter_candidates(population, cell_lines)
        ...
        while len(final_candidates)==0 and prepare_trying_times>=0:
            for batch_idx in range(0, population.size, self.batch_size):  # Optimize batch_size sequences at a time
                ...
                for _ in range(self.n_rounds):
                    optimized_batch_population = self._optimize_batch(batch_population, objectives, cell_lines_to_optimize)
                    ...
                    if optimized_batch_population.is_empty:
                        break
                    optimized_batch_population = self._filter_candidates(optimized_batch_population, cell_lines_to_optimize)
        ...
        if len(final_candidates) !=0:
            ...
            final_population = self._filter_candidates(population, cell_lines_to_optimize)
    ...
    def _filter_candidates(self, population: Population, cell_lines_to_optimize: List[str]) -> Population:
        ...
        filtering_cell_lines = ['K562', 'HepG2', 'GM12878' , 'SKNSH', 'A549']
        ...
        population = self.evaluate_population(population, filtering_objectives)
    ...
    def _optimize_batch(self, population: Population, objectives: List[Objective], cell_lines_to_optimize) -> Population:
        # Optimize the batch
        ...
        population = self.evaluate_population(population, objectives)
    ...
    def _construct_prompt(self, population: Population, objectives: List[Objective], cell_lines_to_optimize: List[str]) -> str:
        ...
        if cell_line.lower() == 'hepg2':
            ...
            cell_line = 'K562'
        elif cell_line.lower() == 'gm12878':
            cell_line = 'GM12878'
        elif cell_line.lower() == 'a549':
            cell_line = 'A549'
