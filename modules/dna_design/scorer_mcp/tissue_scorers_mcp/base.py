...
class Scorer(BaseScorer):
    ...
    def _predict_expression(self, dna_sequences: Tuple[str]) -> List[List[Optional[float]]]:
        ...
        # Process sequences in batches to avoid CUDA out of memory
        all_predictions = []
        for i in range(0, len(tmp_sequences), self.batch_size):
            batch_sequences = tmp_sequences[i:i + self.batch_size]
            batch_predictions = self._predict_batch(batch_sequences)
            all_predictions.append(batch_predictions)

        # Concatenate all batch predictions
        import numpy as np
        pred_data = np.concatenate(all_predictions, axis=0)
        ...
        for idx in valid_indices:
            result[idx] = list(pred_data[k, :])
            k += 1

        return result

    def _predict_batch(self, batch_sequences: List[str]):
        """Predict expression for a batch of sequences with CUDA error handling."""
        df_raw = pd.DataFrame({'seq': batch_sequences})
        ...
        try:
            ...
            if "out of memory" in str(e).lower() and self.device == 'cuda':
                logger.warning(f"CUDA out of memory, falling back to CPU for batch of size {len(batch_sequences)}")
                # Clear CUDA cache and switch to CPU
    ...
    def score_hepg2_tissue_expression(self, samples: List[str]) -> List[Optional[float]]:
    ...
    def score_k562_tissue_expression(self, samples: List[str]) -> List[Optional[float]]:
    ...
    def score_SKNSH_tissue_expression(self, samples: List[str]) -> List[Optional[float]]:
    ...
    def score_a549_tissue_expression(self, samples: List[str]) -> List[Optional[float]]:
    ...
    def score_gm12878_tissue_expression(self, samples: List[str]) -> List[Optional[float]]:
