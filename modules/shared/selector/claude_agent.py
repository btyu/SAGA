...
from claude_agent_sdk import ClaudeSDKClient, ClaudeAgentOptions, CLINotFoundError, ProcessError, CLIJSONDecodeError
import pickle
import subprocess
...
class ClaudeAgent:
    ...
    def __init__(
        ...
        docker_image: str = "btyu24/scileo:v5-claude",
        ...
        Args:
            ...
            mcp_servers: MCP server configurations. Dict mapping server names to their configs.
                        Example: {"tooluniverse": {"command": "uv", "args": ["--directory", "/opt/tooluniverse-env", "run", "tooluniverse"]}}
        """
        ...
        if self.mcp_servers:
            logger.debug(f"ClaudeAgent configured with {len(self.mcp_servers)} MCP server(s): {list(self.mcp_servers.keys())}")
        else:
            logger.debug("ClaudeAgent configured without MCP servers")
    ...
    async def _run_native_async(
        ...
        try:
            ...
            async with ClaudeSDKClient(options=options) as client:
                # Note: MCP tools are loaded internally by the SDK but not directly accessible
                # via Python API. The MCP server will log tool loading in its output.
                # To verify tools are available, check the MCP server logs or use
                # ListMcpResources tool within Claude's execution.
                if self.mcp_servers:
                    logger.debug("MCP servers initialized. Tools will be available to Claude during execution.")
    ...
    def _run_in_docker(
        ...
        # Include both Claude Code credentials and ToolUniverse API keys
        # Note: Only variables that are set on the host will be passed to the container
        for key in ("ANTHROPIC_API_KEY", "CLAUDE_CODE_USE_BEDROCK", "AWS_ACCESS_KEY_ID",
                   ...
                   # ToolUniverse API keys (from .env.template in tooluniverse-env)
                   "OPENAI_API_KEY", "AZURE_OPENAI_API_KEY", "HF_TOKEN",
                   "USPTO_API_KEY", "USPTO_MCP_SERVER_HOST",
                   "BOLTZ_MCP_SERVER_HOST", "TXAGENT_MCP_SERVER_HOST",
        ...
        environment.update({
            ...
            'CLAUDE_MCP_SERVERS': json.dumps(self.mcp_servers) if self.mcp_servers else '{}',
