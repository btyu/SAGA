...
# Monkey patch for scipy float16 compatibility issue
import scipy.sparse
import numpy as np
...
class MLPClassifier(pl.LightningModule):
    ...
    def __init__(
        ...
        self.task_indices = (
            task_indices if task_indices is not None else list(range(num_tasks))
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            f"train/loss_fold{self.fold_index}",
            loss,
            prog_bar=True,
            on_step=False,
            on_epoch=True,
    ...
    def _update_split_metrics(
        ...
        for task_idx in range(self.num_tasks):
            ...
            if unique_vals.numel() >= 2:
                ...
                metrics_container["auprc"][task_idx].update(y_prob, y_true.int())
                metrics_container["auroc"][task_idx].update(y_prob, y_true.int())
            ...
            metrics_container["precision"][task_idx].update(y_pred, y_true.int())
    ...
    def _compute_split_metrics(self, split: str) -> Dict[str, float]:
        ...
        for task_idx in range(self.num_tasks):
            ...
            # Compute AUPRC
            try:
                auprc_val = metrics_container["auprc"][task_idx].compute().item()
            ...
            # Compute AUROC
            try:
                auroc_val = metrics_container["auroc"][task_idx].compute().item()
            ...
            try:
                prec_val = metrics_container["precision"][task_idx].compute().item()
                per_task_values["precision"].append(prec_val)
                per_task_metrics[f"precision_task{actual_task_idx}"] = prec_val
            except (RuntimeError, ValueError):
                per_task_metrics[f"precision_task{actual_task_idx}"] = float("nan")
            ...
            try:
                ...
                per_task_metrics[f"recall_task{actual_task_idx}"] = float("nan")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self._compute_masked_bce_loss(logits, y, mask)
        self.log(
            f"val/loss_fold{self.fold_index}",
            loss,
            prog_bar=False,
            on_step=False,
            on_epoch=True,
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if "auprc" in macro and macro["auprc"] == macro["auprc"]:  # not NaN
            self.log(
                f"val/auprc_avg_fold{self.fold_index}",
                macro["auprc"],
                prog_bar=True,
                on_step=False,
                on_epoch=True,
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = self._compute_masked_bce_loss(logits, y, mask)
        self.log(
            f"test/loss_fold{self.fold_index}",
            loss,
            prog_bar=False,
            on_step=False,
            on_epoch=True,
    ...
    def on_test_epoch_end(self) -> None:
        ...
        for k, v in macro.items():
            if v == v:  # not NaN
                self.log(
                    f"test/{k}_fold{self.fold_index}",
                    v,
                    prog_bar=False,
                    on_step=False,
                    on_epoch=True,
...
class MinimolScorers:
    """Collection of Minimol-based scoring functions for antibacterial activity.

    This class preloads two types of models at initialization:
    - Gram-negative bacteria ensemble (4 tasks: A_baumanii, E_coli, K_pneumoniae, P_aeruginosa)
    - N_gonorrhoeae ensemble (single task)

    Current score thresholds for F1 score maximization:
    - A_baumanii: 0.53
    - E_coli: 0.15
    - K_pneumoniae: 0.13
    - P_aeruginosa: 0.12
    ...
    def __init__(self):
        ...
        # Collect model paths from minimol_antibiotics directory
        self._gram_negative_paths = self._collect_gram_negative_paths(models_dir)
        self._gonorrhea_paths = self._collect_gonorrhea_paths(models_dir)

        if not self._gram_negative_paths:
            raise FileNotFoundError("No gram_negative model checkpoints found.")
        if not self._gonorrhea_paths:
            raise FileNotFoundError("No gonorrhea model checkpoints found.")

        # Preload model ensembles
        self._gram_negative_models = self._load_models(self._gram_negative_paths)
        self._gonorrhea_models = self._load_models(self._gonorrhea_paths)

        # Initialize featurizer immediately (not lazily) to avoid Hydra re-initialization errors
        # Minimol calls hydra.initialize() which can only happen once per process
        self._featurizer = Minimol(batch_size=64)

    def _collect_gram_negative_paths(self, models_dir: Path) -> List[str]:
    ...
    def _collect_gonorrhea_paths(self, models_dir: Path) -> List[str]:
    ...
    def _load_models(self, model_paths: List[str]) -> List[MLPClassifier]:
        """Load all models from the given paths."""
        models = []
        for path in model_paths:
            model = MLPClassifier.load_from_checkpoint(path, map_location="cpu")
            model.eval()
            models.append(model)
        return models

    def _get_featurizer(self):
        """Get featurizer (already initialized in __init__)."""
    ...
    def _featurize_with_skips(
        ...
        def _recursive_featurize(smiles_sublist, positions):
            try:
                ...
                lf, ls, lp = _recursive_featurize(smiles_sublist[:mid], positions[:mid])
                rf, rs, rp = _recursive_featurize(smiles_sublist[mid:], positions[mid:])
    ...
    def _predict_ensemble(
        self, features: torch.Tensor, models: List[MLPClassifier]
    ) -> torch.Tensor:
    ...
    def _predict_on_candidates(
        self, candidates: List[Candidate], models: List[MLPClassifier]
    ) -> List[float]:
        """Predict on candidates using ensemble of models."""
        valid_smiles = []
        valid_indices = []
        ...
        features, kept_smiles, kept_positions = self._featurize_with_skips(valid_smiles)
        ...
        # Map predictions back to original candidates
        results: List[float] = [0.0] * len(candidates)
        for i, (kept_idx, pred) in enumerate(zip(kept_positions, ensemble_preds)):
            original_idx = valid_indices[kept_idx]
            results[original_idx] = float(
                pred[0]
            )  # Take first task for single-task models

        return results

    def _predict_on_candidates_multitask(
        self, candidates: List[Candidate], models: List[MLPClassifier], task_idx: int
    ) -> List[float]:
        """Predict on candidates using ensemble of multi-task models for specific task."""
        valid_smiles = []
        valid_indices = []
    ...
    def score_acinetobacter_baumanii(
        ...
        return self._predict_on_candidates_multitask(
            candidates, self._gram_negative_models, task_idx=0
        )
    ...
    def score_escherichia_coli(
        ...
        return self._predict_on_candidates_multitask(
            candidates, self._gram_negative_models, task_idx=1
        )
    ...
    def score_klebsiella_pneumoniae(
        ...
        return self._predict_on_candidates_multitask(
            candidates, self._gram_negative_models, task_idx=2
        )
    ...
    def score_pseudomonas_aeruginosa(
        ...
        return self._predict_on_candidates_multitask(
            candidates, self._gram_negative_models, task_idx=3
        )
    ...
    def score_neisseria_gonorrhoeae(
        ...
        return self._predict_on_candidates(candidates, self._gonorrhea_models)


if __name__ == "__main__":
    # Simple smoke test for MinimolScorers
    from scileo_agent.core.registry import get_scorer, list_scorers

    print("Available scorers:")
    for scorer_name in list_scorers():
        if "minimol" in scorer_name:
            print(f"  {scorer_name}")

    # Test SMILES - use the working example from user
    test_smiles = [
        "CC(C)C(=O)O",  # Working example from user
        "CN1CCN(c2c(F)cc3c(c2F)-n2c(c(CC(=O)O)c4ccc(Cl)cc42)CC3)CC1",  # Complex antibiotic
        "CCO",  # Simple molecule
        "N#Cc1scnc1Cl",  # Motif we discovered
    ]

    test_candidates = [Candidate(representation=smiles) for smiles in test_smiles]
    ...
    for scorer_name in scorer_names:
        ...
        results = scorer(test_candidates)
        print(f"\n{scorer_name}: {results}")
