...
METHOD_COLORS = {
    ...
    "SciLeo_level0_iter1": "#1F4E5F",
    "SciLeo_no_llm_iter1": "#2A5F6F",
    "REINVENT4": "#A23B72",
    "NatureLM": "#F18F01",
    "MolT5": "#C73E1D",
    "TextGrad": "#6A994E",
...
def load_llm_mutation(filename, target):
    ...
    if "toxicity_safety_chemprop" in df.columns:
...
def load_scileo_seed(filename, target):
    ...
    if needs_computation:
        ...
        if (
            ...
            and "toxicity_safety_chemprop" not in df.columns
        ):
            df["toxicity_safety_chemprop"] = df[
        ...
        # Re-sort the top 100 diverse molecules by aggregate (or aggregated_score fallback)
        final_sort_column = "aggregate" if "aggregate" in df.columns else "aggregated_score"
        if final_sort_column in df.columns:
            df = df.sort_values(final_sort_column, ascending=False).reset_index(drop=True)
    ...
    if (
        ...
        df["toxicity_safety_chemprop"] = df["toxicity_safety_chemprop"]
...
def load_baseline_data(method, target, limit_reinvent=True):
    ...
    # Ensure the final dataframe is sorted by aggregate (or aggregated_score fallback)
    final_sort_column = "aggregate" if "aggregate" in df.columns else "aggregated_score"
    if final_sort_column in df.columns:
        df = df.sort_values(final_sort_column, ascending=False).reset_index(drop=True)
...
def generate_count_table(target):
    ...
    if target == "ecoli":
        ...
        scileo_files = [
            ("kp_level0_iter1_20251122_all_molecules.csv", "level0_iter1"),
            ("kp_no_llm_iter1_20251124_all_molecules.csv", "no_llm_iter1"),
        ]
    else:
        scileo_files = []
        ...
        try:
            ...
            if method.startswith("SciLeo_"):
                ...
                if target == "ecoli":
                    ...
                    filename_map = {
                        "level0_iter1": "kp_level0_iter1_20251122_all_molecules.csv",
                        "no_llm_iter1": "kp_no_llm_iter1_20251124_all_molecules.csv",
                    }
                    filename = filename_map.get(method_suffix)
                    if not filename:
                        raise ValueError(f"Unknown method suffix: {method_suffix}")
...
def main():
    ...
    # Process both targets (skip ecoli if files don't exist)
    for target in ["kpneumoniae"]:  # Only process kpneumoniae for now
