...
def plip_score_computation(dockingpath_list):
    w1,w2,w3 = 0.5, 0.5, 0.0
    protein = parmed.load_file(base_dir+f'/data/pdb/MPRO.pdb')
    final_score_list = []
    for dockingpath in dockingpath_list:
        input_sdf_file = dockingpath

        # Read molecules from the SDF file
        supplier = Chem.SDMolSupplier(input_sdf_file)

        # Iterate through each molecule and write to a separate PDB file
        for _, mol in enumerate(supplier):
            if mol is not None:  # Ensure the molecule was successfully loaded
                output_pdb_file = dockingpath.replace(".sdf", ".pdb")
                Chem.MolToPDBFile(mol, output_pdb_file)
        ligand_dock = parmed.load_file(output_pdb_file)
        complex_dock = protein + ligand_dock
        savepath = output_pdb_file.replace(".pdb", "complex_dock.pdb")
        complex_dock.save(savepath, overwrite=True)

        report_dock = plip_analyze.plip_analyze_single_frame(savepath, mol_name='UNL')
#         print(report_dock)
        final_score = 0
        bond_list = ["hydrogen_bond/HIS/161/A", "hydrogen_bond/GLU/164/A", "pi_stack/HIS/39/A"]

        if bond_list[0] in report_dock:
            final_score += w1*1
        if bond_list[1] in report_dock:
            final_score += w2*1
        if bond_list[2] in report_dock:
            final_score += w3*1
...
if __name__ == "__main__":
    ...
    # protein_path = "modules/small_molecule_drug_design/data/pdb/DRD2.pdb"
