    ...
    for scorer_name, scorer_info in available_scorers.items():
        ...
        def create_wrapper(method, name):
            def wrapper_func(*args, **kwargs):
                ...
                print(f"[ARTHOR] Starting {name} scoring for {num_samples} samples", file=sys.stderr, flush=True)
                sys.stderr.flush()
                
                # Create a fresh instance for each call
                fresh_instance = scorer_class()
                # Get the specific method from the fresh instance
                fresh_method = fresh_instance.scorers[name]['method']
                
                try:
                    result = fresh_method(*args, **kwargs)
                    print(f"[ARTHOR] Completed {name} scoring for {num_samples} samples", file=sys.stderr, flush=True)
                    sys.stderr.flush()
                    return result
                except Exception as e:
                    print(f"[ARTHOR] Error in {name}: {e}", file=sys.stderr, flush=True)
                    sys.stderr.flush()
                    raise
