...
class ScorerDockerRunner:
    ...
    def detect_cuda_support(self) -> bool:
        ...
        except (subprocess.TimeoutExpired, subprocess.CalledProcessError, FileNotFoundError, UnicodeDecodeError) as e:
    ...
    def run_in_docker(
        ...
        """
        if transport == 'stdio':
            warnings.warn("`stdio` transport may not work with docker run. Use streamable-http instead.")
        ...
        if no_gpu:
            ...
            logger.info("CUDA support not detected, skipping GPU access in Docker container")
        
        # Prepare container configuration
        volumes = {
            str(module_dir): {'bind': '/workspace/scorer', 'mode': 'rw'}
        }
        ...
        if transport == "streamable-http":
            mcp_command += f" --host={host} --port={port}"
        ...
        container_config = {
            ...
            'network_mode': 'host',  # Use host networking for MCP server
        ...
        try:
            logger.info(f"Starting MCP server in Docker container with transport: {transport}")
            if transport == "streamable-http":
            ...
            logger.info(f"Started MCP server container: {self.current_container.short_id}")
...
Examples:
  ...
  python -m scorer --local --transport stdio            # Run locally with stdio transport
    ...
    if args.local:
        ...
        run_in_docker(args.transport, extra_args, args.host, args.port, args.image, args.no_gpu)
